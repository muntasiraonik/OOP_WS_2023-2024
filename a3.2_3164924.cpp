...
ostream &operator<<(ostream &os, year &year)
{
    ...
    os << "year: " << year.year_val;
    os << " unit: (" << year.unit_of_values_stored << ")" << endl;
...
year operator-(year &f, year &s)
{
    ...
    {
        cerr << "Error: Attributes of operands do not match in subtraction operator." << endl;
        exit(EXIT_FAILURE);
    }
}

year operator*(float factor, const year &y)
{
    year result = y;
...
void set_unit(year &y,  string new_unit)
{
    y.unit_of_values_stored = new_unit;
}
...
int main()
{
    ...
    while (true)
    {
        ...
        case 'c':
        {
            ...
            if (choice == 'a')
            {
                set_unit(actual, new_unit);
            }
            else
            {
                set_unit(total, new_unit);
            }
        }
        ...
        case 'v':
            ...
            cout << "sum actual = " << fixed << setprecision(2) << sum(actual)
                 << " " << actual.unit_of_values_stored << endl;
            cout << "sum total = " << fixed << setprecision(2) << sum(total)
                 << " " << total.unit_of_values_stored << endl;
            break;
        case 'o':
            cout << actual;
            break;
