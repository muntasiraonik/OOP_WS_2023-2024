...
void read_from_file(const string &filename, char separator, household *households[])
{
    ...
    while (getline(inFile, line))
    {
        ...
        if (line.find("A4") != string::npos)
        {
            ...
            getline(iss, task_name, ';');
            getline(iss, max_households_str, ';');
            getline(iss, city, ';');
            getline(iss, power_price_str, ';');
        ...
        if (line.find("household") != string::npos)
        {
            ...
            getline(iss, type, ';');
            getline(iss, householdNumberStr, ';');
            getline(iss, city, ';');
            getline(iss, hotWaterElectricStr, ';');
            getline(iss, numberOfPeopleStr, ';');
            getline(iss, squareMetersStr, ';');
